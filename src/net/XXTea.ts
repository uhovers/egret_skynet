class XXTea {
    ...
    constructor() {
        this.keyString = "a4b1a4d6fa671a7e89687326211d5b25";  //xxtea密钥 一般16位（也可以不是16位）</span>
    }
    ...
    encrypt(str) {
        ...
        // return encode_base64(str);//加密过后用base64编码</span>
        return str
    }
...
function utf16ToUtf8(str) { //utf16转化utf8 解决中文乱码问题</span>
    var out, i, len, c;
    out = "";
    len = str.length;
    for (i = 0; i < len; i++) {
...
function long2str(v, w) { //解密过程中将一个32位 数据转换成4个8位 的数据</span>
    var vl = v.length;
    var n = (vl - 1) << 2;
    if (w) {
        var m = v[vl - 1];
...
function str2long(s, w) { //加密过程中将4个8位的数据转化成一个32位的数据</span>
    var len = s.length;
    var v = [];
...
function encode_base64(input) { //base64 编码 </span>
